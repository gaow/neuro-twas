...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	outdir = dirname(out)
  	out = basename(out)
	arg = paste( opt$PATH_gemma , " -miss 1 -maf 0 -r2 1 -rpace 1000 -wpace 1000 -bfile " , input , " -bslmm " , bv_type , " -o " , out, " -outdir ", outdir, sep='' )
	system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
	eff = read.table( paste(outdir,'/',out,".param.txt",sep=''),head=T,as.is=T)
