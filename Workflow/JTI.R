...
jti.modeltrain = function(exp, 
  ...
  lambda_list<-
    fit_st$lambda[max(1,which(fit_st$lambda==fit_st$lambda.min)-10):min(length(fit_st$lambda),which(fit_st$lambda==fit_st$lambda.min)+10)] 
  ...
  colnames(r_matrix)<-
    c('exp_power','dhs_power','lambda','window','r_test','p_test')
  r_matrix$window='1m'
  ...
  #find the best hyper parameters with the largest r_test
